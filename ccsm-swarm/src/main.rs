...
impl SwarmServer {
    fn new() -> Self {
        Self { state: Arc::new(Mutex::new(SwarmState::default())) }
    }
...
impl SwarmServer {
    ...
    /// Returns a JSON array: [{session, window, pane_index, pane_id, process}, ...]
    #[tool(description = "List all tmux panes with session, window, and process info")]
    ...
    /// Pass `lines: N` for explicit line count, or omit for delta mode.
    #[tool(description = "Capture pane output (delta-aware — returns only new content)")]
    async fn swarm_capture(
        ...
        let delta_bytes = state.update_bytes(&resolved, total_bytes);

        if delta_bytes == 0 {
            Ok(String::new())
        } else if delta_bytes >= total_bytes {
            Ok(content)
        } else {
            let start = total_bytes - delta_bytes;
            Ok(content[start..].to_string())
        }
    }

    /// Send text to a pane. Optionally press Enter after the text.
    #[tool(description = "Type text into a pane (optionally press Enter)")]
    ...
    #[tool(description = "Block until a sentinel string appears in pane output")]
    async fn swarm_wait(
        &self,
        ...
        loop {
            if start.elapsed() > timeout {
                return Ok(serde_json::json!({
                    "ok": false,
                    "error": format!("timeout after {}s waiting for sentinel", timeout_secs),
                }).to_string());
            }
    ...
    /// Returns JSON with {panes: [{session, pane_id, process, last_line}], count}.
    #[tool(description = "Consolidated status of all panes or a specific one")]
    async fn swarm_status(
        ...
        for pane in &panes {
            ...
            let content = tmux::capture_pane(&pane.pane_id, Some(10)).unwrap_or_default();
    ...
    #[tool(description = "Broadcast text to all panes")]
    async fn swarm_broadcast(
        ...
        for pane in &panes {
            tmux::send_keys(&pane.pane_id, &text, enter)
                .map_err(|e| McpError::internal_error(e.to_string(), None))?;
            sent.push(pane.pane_id.clone());
        }
    ...
    #[tool(description = "Label a pane for name-based targeting")]
